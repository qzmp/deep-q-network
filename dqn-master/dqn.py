...
class Agent():
    ...
    def build_network(self):
        model = Sequential()
        model.add(Conv2D(filters=32, kernel_size=(8, 8), strides=(4, 4), activation='relu', input_shape=(STATE_LENGTH, FRAME_WIDTH, FRAME_HEIGHT), data_format="channels_first"))
        model.add(Conv2D(filters=64, kernel_size=(4, 4), strides=(2, 2), activation='relu', data_format="channels_first"))
        model.add(Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), activation='relu', data_format="channels_first"))
        model.add(Flatten())
